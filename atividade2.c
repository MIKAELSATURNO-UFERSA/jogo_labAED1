...
typedef struct {
    ...
    int       valor;
    bool      coletada;
    float     tempoColeta; //adicionado pra guardar a coleta 
} Moeda;
...
Moeda *criarMoedas(int quantidade) {
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        m->raio     = 10.0f;
            int raridade = GetRandomValue(0, 9);
    if (raridade == 0) {
...
bool tentarColetar(Moeda *m, Vector2 posJogador, float raioJogador) {
    ...
    if (distancia <= somaRaios) {
        m->coletada = true;
        m->tempoColeta = GetTime();  // adicionado pra guardar o tempo coleta
        return true;
    }
    return false;
}
...
void reposicionarMoeda(Moeda *m) { //adicionaado pra reposicionar as moedas 
    m->pos = (Vector2){ GetRandomValue(30, LARGURA_JANELA - 30),
                         GetRandomValue(30, ALTURA_JANELA - 30) };
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < TOTAL_MOEDAS; i++) { // adicionado pra reposicionar as moedas depois dos 3s 
            Moeda *m = (moedas + i);
            if (m->coletada && (GetTime() - m->tempoColeta >= 3)) {
                reposicionarMoeda(m);
            }
        }
