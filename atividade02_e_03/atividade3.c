...
typedef struct {
    int cura;
    bool envenenada; // adicionado pra que se for verdadeiro a poção seja venenosa
} DadosPocao;
...
Item *criarItens(int quantidade) {
    ...
    int veneno = GetRandomValue(0, 9);
    if (veneno < 3) {
        it->dados.pocao.cura = -GetRandomValue(5, 15);
        it->dados.pocao.envenenada = true;
    } else {
        it->dados.pocao.cura = GetRandomValue(10, 30);
        it->dados.pocao.envenenada = false;
    }
        } else {
            it->dados.escudo.absorcao = GetRandomValue(1, 5);
        }
    }
    return itens;
}
...
void aplicarItem(Jogador *j, Item *item) {
    switch (item->tipo) {
        ...
        case ITEM_POCAO:
            ...
            if (j->vida < 0) j->vida = 0; // pra vida não ficar negativa
    }
    item->coletado = true;
}
...
void desenharItem(Item *item) {
    ...
    switch (item->tipo) {
        case ITEM_ARMA:
            ...
            break;
        case ITEM_POCAO:
            if (item->dados.pocao.envenenada) {
                cor = PURPLE; 
            } else {
                cor = GREEN;
            }
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            DrawText(TextFormat("Armadura: %d", jogador.armadura), 10, 68, 22, DARKGRAY);
            DrawText(TextFormat("Vida: %d   Dano: %.1f", jogador.vida, jogador.dano), 10, 10, 22, DARKGRAY);
            DrawText("Vermelho = arma (aumenta dano) | Verde = pocao (cura) | Roxo = poção venenosa", 10, 34, 18, GRAY);
            DrawText("Dourado = escudo", 10, 55, 18, GRAY);
